 ...
 */
export function isSeparatingAxis(aPos: Vector, bPos: Vector, aPoints: Array<Vector>, bPoints: Array<Vector>, axis: Vector, testResult: CollisionResult) {
    ...
    if (testResult) {
        ...
        if (absOverlap < testResult.depth) {
            testResult.depth = absOverlap;
            testResult.normal.set(axis);
            if (overlap < 0) {
                testResult.normal.reverse();
            }
        }
    }
    T_VECTORS.push(offsetV);
...
export function testPolygonPolygon(a: Polygon, b: Polygon, testResult: CollisionResult) {
