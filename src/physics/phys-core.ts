...
export function positionalCorrection(A: PhysMaterial & PhysProperty, B: PhysMaterial & PhysProperty, result: CollisionResult)  {
    const percent = 0.2; // usually 20% to 80%
    const slop = 0.01; // usually 0.01 to 0.1
    const correction = result.normal.clone();
    correction.scl(Math.max( result.depth - slop, 0.0 ) / (A.invMass + B.invMass) * percent);
